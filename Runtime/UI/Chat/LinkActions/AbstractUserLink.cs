...
namespace Amilious.Core.UI.Chat.LinkActions {
    ...
    public abstract class AbstractUserLink : AbstractNetworkLink {
        ...
        public sealed override bool ShowMenuItem(IChatBox chatBox, string text, string[] linkArgs) {
            return TryGetUser(linkArgs, out var user) && ShowUserMenuItem(chatBox, user);
        }

        /// <inheritdoc />
        public sealed override void Action(IChatBox chatBox, string text, string[] linkArgs) {
            if(TryGetUser(linkArgs, out var user)) UserAction(chatBox,user);
        }
        ...
        protected abstract bool ShowUserMenuItem(IChatBox chatBox, UserIdentity user);
        ...
        /// <param name="chatBox">The chat display.</param>
        /// <param name="user">The user that the menu is for.</param>
        protected abstract void UserAction(IChatBox chatBox, UserIdentity user);
